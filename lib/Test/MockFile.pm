...
# Shared permission check for open/sysopen paths.
# $is_new: true if the file is being created (contents were undef before O_CREAT).
# $func_name: 'open' or 'sysopen' (for autodie).
# $rw: read/write mode string (e.g. 'r', 'w', 'rw', 'wa').
# Returns 1 if allowed, 0 if denied (sets $! = EACCES and throws if autodie).
...
sub __sysopen (*$$;$) {
    ...
    # Track whether this is a new file BEFORE O_CREAT populates contents.
    # Used by _check_open_perms to decide file-perms vs parent-dir-perms.
